 ...
 * 互斥体 API 函数
 ** DEFINE_MUTEX(name)
 * @brief 定义并初始化一个 mutex 变量
 ** void mutex_init(mutex *lock)
 * @brief 初始化 mutex
 ** void mutex_lock(struct mutex *lock)
 * @brief 获取 mutex,也就是给 mutex 上锁如果获取不到就进休眠
 ** void mutex_unlock(struct mutex *lock)
 * @brief 释放 mutex,也就给 mutex 解锁
 ** int mutex_trylock(struct mutex *lock)
 * @brief 尝试获取 mutex,如果成功就返回 1,如果失败就返回 0
 ** int mutex_is_locked(struct mutex *lock)
 * @brief 判断 mutex 是否被获取,如果是的话就返回1,否则返回 0
 ** int mutex_lock_interruptible(struct mutex *lock)
 * @brief 使用此函数获取信号量失败进入休眠以后可以被信号打断
 */

#define GPIOLED_NAME  "mutex" // 设备名
