 ...
 * @brief 用于申请一个 GPIO 管脚
 * @param gpio 要申请的 gpio 标号，使用 of_get_named_gpio 函数从
 * 设备树获取指定 GPIO 属性信息，此函数会返回这个 GPIO 的标号
 * @param label 给 gpio 设置个名字
 * @return 0，申请成功；其他值，申请失败
 *
 ** void gpio_free(unsigned gpio)
 * @brief 如果不使用某个 GPIO 了，那么就可以调用 gpio_free 函数进行释放
 * @param gpio 要释放的 gpio 标号
 *
 ** int gpio_direction_input(unsigned gpio)
 * @brief 用于设置某个 GPIO 为输入
 * @param gpio 要设置为输入的 GPIO 标号
 * @return 0，申请成功；其他值，申请失败
 *
 ** int gpio_direction_output(unsigned gpio, int value)
 * @brief 用于设置某个 GPIO 为输出
...
MODULE_DESCRIPTION("gpio led driver module");