 ...
 * @brief 使用 tasklet_init 函数初始化 tasklet
 * @param t 要初始化的 tasklet
 * @param func tasklet 的处理函数
 * @param data 要传递给 func 函数的参数
 *
 ** DECLARE_TASKLET(name, func, data)
 * @brief 使用宏DECLARE_TASKLET来一次性完成tasklet的定义和初始化
 * @param name tasklet_struct 类型的时候变量
 * @param func tasklet 的处理函数
 * @param data 要传递给 func 函数的参数
 *
 ** void tasklet_schedule(struct tasklet_struct *t)
 * @brief 使 tasklet 在合适的时间运行
...
struct key_irq_desc {
    ...
    struct tasklet_struct tasklet;      // 使用tasklet实现下半部机制
};
...
static irqreturn_t key_handler(int irq_num, void* dev_id)
{
    ...
    // 调度 tasklet
    tasklet_schedule(&dev->key_irq[0].tasklet);
...
{
    struct interrupt_dev* dev = (struct interrupt_dev*)data;
...
static int keyio_init(struct interrupt_dev* dev)
{
    ...
    for (i = 0; i < KEY_NUM; i++) {
        ...
        // 初始化 tasklet 这里所有的按钮都是用的同一个回调 ,因该是每一个按键各自对应一个回调
        tasklet_init(&dev->key_irq[i].tasklet, key_tasklet, (volatile unsigned long)dev);
    }
