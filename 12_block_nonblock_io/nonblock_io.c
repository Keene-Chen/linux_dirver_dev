 ...
 * @param wait 结构体 poll_table_struct 类型指针,由应用程序传递进来的一般将此参数传递给poll_wait函数
 * @return 向应用程序返回设备或者资源状态
 * @retval POLLIN 有数据可以读取
 * @retval POLLPRI 有紧急的数据需要读取
 * @retval POLLOUT 可以写数据
 * @retval POLLERR 指定的文件描述符发生错误
 * @retval POLLHUP 指定的文件描述符挂起
 * @retval POLLNVAL 无效的请求
 * @retval POLLRDNORM 等同于 POLLIN，普通数据可读
 */
...
static ssize_t interrupt_read(struct file* filp, char __user* buf, size_t count, loff_t* offt)
{
    ...
    if (filp->f_flags & O_NONBLOCK) { // 应用程序使用非阻塞方式读
        if (atomic_read(&dev->key_release) == 0) {
            return -EAGAIN;
        }
    }
    else { // 应用程序使用阻塞方式读
        wait_event_interruptible(dev->read_wait, atomic_read(&dev->key_release));
    }
...
unsigned int interrupt_poll(struct file* filp, struct poll_table_struct* wait)
{
    int mask                  = 0;
    struct interrupt_dev* dev = (struct interrupt_dev*)filp->private_data;
    ...
    // 判断驱动程序是否可读
    if (atomic_read(&dev->key_release)) { // 按键按下可读
